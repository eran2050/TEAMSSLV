...
import net.voaideahost.sslv.dao.ads.NewAdDAO;
...
public class NewAdPageController extends AbstractController {
	...
	protected ModelAndView handleRequestInternal(HttpServletRequest request,
		...
		String outStringStart = VAL_EMPTY;
		String outStringFinish = VAL_EMPTY;
		String outStringMiddle = VAL_EMPTY;

		NewAdPageModel m = new NewAdPageModel();
		m.setAppVer(VAL_APP_VERSION);
		...
		// However as future project evolution - this should be changed to
		// dynamical
		ArrayList<NewAdPageParams> paramList1 = new ArrayList<NewAdPageParams>();
		// PRICE initialisation > read from Http_request > write to temporary
		// variable
		...
		// Condition initialisation > read from Http_request > write to
		// temporary variable
		...
		// Other initialisation > read from Http_request > write to temporary
		// variable
				...
				if (m.getName().equals(VAL_EMPTY)) {
					...
					if (m.getSavedStatus() == null) {
						...
						if (hs.getAttribute("BLOCKED") == null) {
							...
							if (!dao.setNewAd(m.getName(), m.getUserName(),
									m.getParamList())) {
						...
						// nevertheless the SAVE was blocked or not - going
						// further
						...
						outStringStart = "<table class='ml'>" + "<tr>"
								...
								+ "<th class='ml'>Status</th>" + "</tr>"
								+ "<tr>" + "<td>" + m.getUserName() + "</td>"
								+ "<td>" + m.getName() + "</td>" + "<td>"
								+ m.getSavedStatus() + "</td>" + "</table>";
						...
						for (NewAdPageParams c : m.getParamList()) {
							...
							outStringMiddle = outStringMiddle
									+ "<tr><td class='ml2'>" + c.getParamName()
									+ "</td>" + "<td class='ml2'>"
